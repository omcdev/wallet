...
namespace OmniCoin.Miner
{
    class Program
    {
        ...
        static void Run(string[] args)
        {
            ...
            app.Command("mining", command =>
            {
                ...
                command.OnExecute(async () =>
                {
                    if (nameArgument != null && !string.IsNullOrEmpty(nameArgument.Value) && addressArgument != null && !string.IsNullOrEmpty(addressArgument.Value))
                    {
                        ...
                        if (chainStatus == null)
                        {
                            app.Out.WriteLine("there is something wrong with the api, you should check the OmniCoin");
                            return;
                        }
                        //验证本地区块高度和网络区块高度
            ...
            /*
            if (args.Length > 1 && args[0].ToLower() == "qstpay" && IsContainsCommand(args[1]))
            {
